...
public class HardAI extends MiniMaxAI {
    ...
    public HardAI(int time, Random r) {
        super(time, r);
    }

    public HardAI(int time, int seed) {
        super(time, seed);

    }

    public HardAI(int time) {
        super(time);
    }
    ...
    public Move selectMove(HashMap<Move, Integer> movesMap, Kube k3) {
        ...
        if (entries.size() >= 2 && entries.get(entries.size() - 2).getValue() > 0){
